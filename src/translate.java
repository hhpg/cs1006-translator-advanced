...
public class translate {
    ArrayList<word> espTokens = new ArrayList<>();
    private HashMap<String, word> espDict = new HashMap<String, word>();
    ...
    public static void main(String[] args) throws IOException {
        ...
        System.out.println("SPANISH TO ENGLISH TRANSLATOR");
        System.out.println("------------------------------");
    ...
    private void readEspDict() throws IOException {
        BufferedReader fileRead = new BufferedReader(new InputStreamReader(new FileInputStream("engDict.csv"), "UTF8"));

        String key = ""; /* String representing the key that will map the Spanish translation of a word to the Hash Map */
        String engWord = ""; /* String representing the translated Spanish word */
    ...
    private void getTypes() {
        String adj;
        for(int i = 0; i < espTokens.size(); i++) {
            for (String key : dict.keySet()) {
                if (espTokens.get(i).getName().equals(dict.get(key).getName())) {
                    word x = new word();
            ...
            if (espTokens.get(i).getName().toLowerCase().equals("la")) {
                espTokens.get(i).setName("el");
                espTokens.get(i).setType("article");
            } else if (espTokens.get(i).getName().equals("una")) {
                espTokens.get(i).setName("un");
                espTokens.get(i).setType("article");
            }

            if (espTokens.get(i).getType().isEmpty()) { /* feminine;  Rule 1 */
                if (!dict.containsKey(espTokens.get(i).getName())) {
                    adj = espTokens.get(i).getName();
                    int size = adj.length();
                    String endingChar;
                    if (adj.endsWith("a")) {
                        endingChar = "o";
                    ...
                    adj = adj.substring(0, size - 1) + endingChar;
                    espTokens.get(i).setName(adj);
                    espTokens.get(i).setType("adjective");
                }
            }
